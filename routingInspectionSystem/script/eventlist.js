apiready = function() {
    ...
    var addMessage = function(messageId, name, status, types, leftTime, repTime, information, road, handlename, reportuserid, funcdescide) {
        ...
        if (showStatus == "待接单" && reportuserid == info.user.userid) {
    ...
    var recoveryfrommyrequest = function() {
        hidelist(recovery);
        hidelistforMy(function(container){
            checkmyhandle(container);
            var page = addcheck ? addcheck : 1;
            var childs = container.children;
            var checkuser = childs[9].innerHTML; //用户id比对。
            if (page == 1 && checkuser == info.user.userid && !myrequest){
                container.setAttribute("hidden", "hidden");
            }
        });
